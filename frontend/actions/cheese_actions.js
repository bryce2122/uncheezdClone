...
export const createCheese = data => dispatch => {
  return CheeseAPIUtil.createCheese(data).then(cheese => (
    dispatch(receiveACheese(cheese))
  ), err => {
    return dispatch(receiveCheeseErrors(err.responseJSON));
  });
...
export const updateCheese = (data,id) => dispatch => {
  console.log("IDA",id);
  return CheeseAPIUtil.updateCheese(data,id).then(cheese => (
    dispatch(receiveACheese(cheese))
  ), err => {
    return dispatch(receiveCheeseErrors(err.responseJSON));
  });
